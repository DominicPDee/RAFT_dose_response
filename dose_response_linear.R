dose_response_linear <- function(df1, LC_in = c(50,99)){
  ...
  data_stan <- list(N=nrow(df1),
                    ...
                    year=df1$Covariate-min(df1$Covariate))

  if(file.exists(paste0("outputs/fits/linear_",unique(df1$rlookup),".rds"))){
    fit <- readRDS(file=paste0("outputs/fits/linear_",unique(df1$rlookup),".rds"))

  } else {

    model <- rstan::stan_model("models/5PL_time_lin_NEW_v2.stan")
    ...
    saveRDS(fit, file=paste0("outputs/fits/linear_",unique(df1$rlookup),".rds"))
    write.csv(modelcheck, file=paste0("outputs/csv/modelcheck_linear_",unique(df1$rlookup),".csv"))
  }
  ...
  saveRDS(LOO_fi, file=paste0("outputs/LOO/linear_",unique(df1$rlookup),".rds"))
  ...
  fit_mort <- tibble(Concentration =mean_mort$concentration,
                     ...
                     #Site=unique(df1$Site), ### ONLY IF PRESENT IN DATA
  ...
  #
  # # 2/ LC ----
  lcx <- function(y,B,E,H,G,year){
    return(exp(((log((((-1)/(y-1))^(1/E))-1))/B)+((H+G*year))))
  }
  #
  Bfit <- rstan::extract(fit)[["B"]]
  Efit <- rstan::extract(fit)[["E"]]
  Hfit <- rstan::extract(fit)[["F"]]
  Gfit <- rstan::extract(fit)[["G"]]
  ...
  sapply(LC_in, function(a){
    ...
    temp = do.call(rbind,
                   sapply(0:(data_stan$Y-1),
                          function(x){
                            data.frame("LC" =
                                         sapply(1:length(Bfit),
                                                function(i){
     lcx(a/100, Bfit[i],Efit[i],Hfit[i],Gfit[i],x)
      }),
      ylu = x+1)
    }, simplify = FALSE))
    ...
    lc_summ <- lc %>%
      group_by(Insecticide, rlookup, Covariate,LC_value) %>%
      summarise(LC_mean=mean(LC, na.rm=T),
                LC_median=median(LC, na.rm=T),
                LC_lower=quantile(LC,0.025, na.rm=T),
                LC_upper=quantile(LC,0.975, na.rm=T))


    write.csv(lc, file=paste0("outputs/csv/linear_LC", a, "_",unique(df1$rlookup),".csv"))
    write.csv(lc_summ, file=paste0("outputs/csv/linear_summ_LC", a, "_", unique(df1$rlookup),".csv"))
  ...
  #
  # # 5/ Save objects----
  write.csv(df1_s, file=paste0("outputs/csv/df1_s_linear_",unique(df1$rlookup),".csv"))
  ggsave(plot,file=paste0("outputs/plots/linear_",unique(df1$rlookup),".png"),
         width = 7,height = 5)
